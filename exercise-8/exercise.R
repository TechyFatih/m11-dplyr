...
setwd('~/GitHub/INFO 201/Exercises/m11/exercise-8')
# Read in the data
...
# Add a column in a dataframe called 'Pulitzer.Prize.Change` that contains the difference in changes
...
filter(pulitzer, Pulitzer.Prize.Winners.and.Finalists..2004.2014 == max(Pulitzer.Prize.Winners.and.Finalists..2004.2014)) %>%
  select(Newspaper)
...
# Q: Which newspapers had a positive increase in daily circulation from 2004 to 2013?
filter(pulitzer, as.numeric(sub('%', '', Change.in.Daily.Circulation..2004.2013)) > 0) %>%
  select(Newspaper, Daily.Circulation..2004, Daily.Circulation..2013, Change.in.Daily.Circulation..2004.2013)
